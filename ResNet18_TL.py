...
from NN_architectures import PretrainedResNet18
...
parser.add_argument('--output_dir','-od', type=str, default='ResNet18_TransferLearning_debug', required=False,help='output directory for figures')
...
parser.add_argument('--alpha','-alpha',type=float,default=0,required=False,help='strength of L1 regularization for the loss')
...
model = PretrainedResNet18(out_dim=out_dim)
print(model)
...
def random_restart_training(device=None, lr=args.lr, nepochs=1000,
             early_stopping_patience=2, alpha=args.alpha,beta=args.blocky_inversion):
    print('\nTRAINING LOOP:')
    if device is None:
        ...
        if epoch % 5 == 0:
            ...
            if mean_val_loss < best_val_loss:
                ...
                if epochs_without_improvement >= early_stopping_patience:
                    ...
                    model = PretrainedResNet18().to(device)  # Reinitialize model
                    optimizer = {
                        'Adam': optim.Adam(model.parameters(), lr=lr),
                        'Nadam': optim.NAdam(model.parameters(), lr=lr),
                        'RMSprop': optim.RMSprop(model.parameters(), lr=lr)
...
'''def learning_curves(epochs_count_train, train_losses, epochs_count_val, val_losses, od=args.output_dir):
    ...
    ax.set_xlabel('Epoch', fontsize=10)  # Taille de la police ajustée
    ax.set_ylabel('Loss', fontsize=10)
    ax.set_yscale('log')  # Echelle logarithmique pour l'axe y
    ax.set_title('Learning Curves', fontsize=12)  # Titre avec une taille de police adaptée
