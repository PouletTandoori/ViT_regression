...
from NN_architectures import PretrainedPVTv2
...
model = PretrainedPVTv2(out_dim=out_dim)
print(model)
...
def random_restart_training(device=None, lr=args.lr, nepochs=1000,
    ...
    print('Device: ', device)
    model = PretrainedPVTv2().to(device)
        ...
        if epoch % 5 == 0:
            ...
            if mean_val_loss < best_val_loss:
                ...
                if epochs_without_improvement >= early_stopping_patience:
                    ...
                    model = PretrainedPVTv2().to(device)  # Reinitialize model
                    optimizer = {
                        'Adam': optim.Adam(model.parameters(), lr=lr),
                        'Nadam': optim.NAdam(model.parameters(), lr=lr),
                        'RMSprop': optim.RMSprop(model.parameters(), lr=lr)
